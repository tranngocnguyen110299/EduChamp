...
namespace TN_academic.Areas.Admin.Controllers
{
    ...
    public class InstructorsController : BaseController
    {
        ...
        {
            var users = db.Users.Include(u => u.UserStatu).Where(u => u.Role.Equals("2"));
        ...
        public ActionResult Edit(string id)
        {
            ...
            Session.Add(Common.CommonConstants.TEMP_INSTRUCTOR_IMAGE, user.Avatar);
            ViewBag.StatusID = new SelectList(db.UserStatus, "StatusID", "StatusName", user.StatusID);
        ...
        public ActionResult Edit([Bind(Include = "Username,FirstName,LastName,Password,Gender,DOB,Phone,Email,Address,Avatar,StatusID,Role,EditedImage")] UserModelForEdit user)
        {
            if (ModelState.IsValid)
            {
                ...
                {
                    user.Avatar = Session[Common.CommonConstants.TEMP_INSTRUCTOR_IMAGE].ToString();
                }
                else
                {
                    ...
                    try
                    {
                        System.IO.File.Delete(Server.MapPath(Session[Common.CommonConstants.TEMP_INSTRUCTOR_IMAGE].ToString()));
                    }
                    catch (Exception){}
                ...
                if (db.SaveChanges() > 0)
                {
                    Session.Remove(Common.CommonConstants.TEMP_INSTRUCTOR_IMAGE);
